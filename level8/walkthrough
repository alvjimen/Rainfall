...
_start
32b   | 64b
esp   | rsp == num args    -> argc
[esp] | [rsp] == first arg -> argv[0]
esp   | rsp == array args  -> argv.
## stack
A. Stack Frame  

    The stack frame is a block of memory that holds:  
        The function’s parameters.  
        Local variables.  
        Saved registers.  
        The return address (so the function knows where to return).
         
    %ebp acts as a fixed reference point to access these values.
     

B. Accessing Parameters and Locals  

    Using %ebp:  
    Return address : [ebp + 4].  
    First parameter : [ebp + 8] (assuming cdecl or similar calling convention).  
    Saved registers : Located below %ebp (e.g., old %edi is at [ebp - 8]).  
    Local variables : Allocated by subtracting from %esp (not shown here).


# For binary explotation
## detect the target binarie, perms ..
## reverse
## exploit
level8@RainFall:~$ ls -lhtra
-rwsr-s---+ 1 level9 users  6.0K Mar  6  2016 level8
...
# Ejecucion
(nil), (nil) 
hola <- loop read from stdin until EOF (Ctrl + D)
(nil), (nil)
# ejecucion con libcalls
 ltrace ./level8
...
let's try the words appear on .rodata
  [     8]  %p, %p ^J   -> # used
  [    11]  auth        -> # global pointer malloc 4 bytes and strcpy heap-overflow
  [    17]  reset       -> # free the malloc of auth
  [    1d]  service     -> # strdup on other global pointer
  [    25]  login       -> # ask for a Password don't clue how access the /bin/sh or system
  [    2b]  /bin/sh     -> interesting path
  [    33]  Password:^J -> may ask for a password stdin ?
  fgets(auth
"auth\n", 128, 0xb7fd1ac0)                      = 0xbffffbf0
...
"service \n", 128, 0xb7fd1ac0)                  = 0xbffffbf0
strdup(" \n")                                         = 0x0804a008
printf("%p, %p \n", 0x804a008, 0x804a0080x804a008, 0x804a008  -> strdup of the string no need of space the same pointer because the code free but not null the pointer.
)             = 22
fgets(login
"login\n", 128, 0xb7fd1ac0)                     = 0xbffffbf0
...
## How to Exploit
# Reviewing the asm and the libcalls look like i need to create a heap string with auth and in the pos 32, 33, 34, 35 != 0
auth malloc just 4 risk segfault and undefined behavior the bad part is that is the ref pointer to pointer arithmetics, Should create one.
ideally then free and create a full string with service that will return the value of the not nulled auth global var
service save in another place and can create more than 4 bytes each time.
fresh run
"auth bla\n"
"reset"
